      ...
      {this.state.cartItems.length>0 ? (<span>
        ...
        <div className={classes.checkout}>
          ...
          {!this.props.checkout && (auth.isAuthenticated()?
            <Button color="secondary" variant="raised" onClick={this.openCheckout}>Book Now</Button>
            :
            <Link to="/signin">
              <Button color="primary" variant="raised">Sign in to checkout</Button>
            </Link>)}
          <Link to='/' className={classes.continueBtn}>
            <Button variant="raised">Continue Booking</Button>
          </Link>
        </div>
      </span>) :
      <Typography type="subheading" component="h3" color="primary">No items added to Booking list.</Typography>
    }
    </Card>)
  }
}
